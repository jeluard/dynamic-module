...
//Add support for friendlyName
    ...
    protected final String findMessageProcessorClassName(final Package modulePackage, final String processorName) {
        return modulePackage.getName()+"."+Strings.capitalize(processorName)+Devkit42Loader.MESSAGE_PROCESSOR_CLASS_SUFFIX;
    }

    protected final String findMessageSourceClassName(final Package modulePackage, final String sourceName) {
        return modulePackage.getName()+"."+Strings.capitalize(sourceName)+Devkit42Loader.MESSAGE_SOURCE_CLASS_SUFFIX;
    }

    protected final String findTransformerClassName(final Package modulePackage, final String transformerName) {
        return modulePackage.getName()+"."+Strings.capitalize(transformerName)+Devkit42Loader.TRANSFORMER_CLASS_SUFFIX;
    }
    ...
    protected final String[] extractMethodParameterNames(final Class<?> generatedClass) {
        ...
        for (final Field field : generatedClass.getDeclaredFields()) {
            ...
            if (!(fieldName.startsWith(Devkit42Loader.PARAMETER_TYPE_FIELD_PREFIX) && fieldName.endsWith(Devkit42Loader.PARAMETER_TYPE_FIELD_SUFFIX))) {
                continue;
            }

            final String parameterName = Strings.uncapitalize(fieldName.substring(Devkit42Loader.PARAMETER_TYPE_FIELD_PREFIX.length(), fieldName.length()-Devkit42Loader.PARAMETER_TYPE_FIELD_SUFFIX.length()));
